if not exists (select * from sys.schemas where name = 'sync')
  exec sys.sp_executesql @stmt=N'create schema [sync]'
go
...
begin
  ...
  *************************************/
  begin try
    ...
    using sync.grupomaterial as source with (nolock) on 
      target.id = source.grupo_id
    when matched then
      update set
  ...
  *************************************/
  begin try
    ...
    using sync.classe as source with (nolock) 
      on target.id = source.classe_id
    when matched then
      update set
        descricao = source.nm_descricao,
        --descricao_touch = ,
  ...
  *************************************/
  begin try
    ...
    using sync.combo as source with (nolock) on 
      target.id = source.combo_id
    when matched then
      update set
        dt_alt = @getDate,
  ...
  *************************************/
  begin try
    ...
    delete sync.movclasse 
    where IT_CLASSE is not null 
      and it_CLASSE not in (select id from dbo.classe)

    delete sync.movclasse 
    where IT_MATERIAL is not null 
      and it_material not in (select id from dbo.material)

    delete sync.movclasse 
    where IT_COMBO is not null 
      and it_combo not in (select id from dbo.combo)
  ...
  *************************************/
  begin try
    ...
    using sync.observacao as source with (nolock) on 
      target.id = source.observacao_id
    when matched then
      update set
        dt_alt = @getDate,
        nome = nm_observacao
  ...
  *************************************/
  begin try
    ...
    using sync.motivocanc as source with (nolock) on 
      target.id = source.motivo_id
    when matched then
      update set
        dt_alt = @getDate,
        codigo = nu_codigo,
  ...
  *************************************/
  ...
  begin
    begin try
      ...
      using sync.tiporecebimento as source with (nolock) on 
        target.id = source.tiporecebimento_Id
      when matched then
        update set
          dt_alt = @getDate,
          codigo = nu_codigo,
