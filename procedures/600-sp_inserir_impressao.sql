    ...
    -- remove as impressoes do perfil de impressao caso o modo de venda tenha mudado
      if (@balcao = 0)
        ...
        where impressao_id = @id
          and perfil_id in (select id from dbo.perfil_impressao where modo_venda_id = 1)

      if (@entrega = 0)
        delete from dbo.perfil_impressao_detalhe
        where impressao_id = @id
          and perfil_id in (select id from dbo.perfil_impressao where modo_venda_id = 2)

      if (@mesa = 0)
        delete from dbo.perfil_impressao_detalhe
        where impressao_id = @id
          and perfil_id in (select id from dbo.perfil_impressao where modo_venda_id = 3)

      if (@ficha = 0)
        delete from dbo.perfil_impressao_detalhe
        where impressao_id = @id
          and perfil_id in (select id from dbo.perfil_impressao where modo_venda_id = 4)

      if (@balcao = 1 or @entrega = 1 or @mesa = 1 or @ficha = 1)
        delete from dbo.perfil_impressao_detalhe
        where impressao_id = @id
          and perfil_id in (select id from dbo.perfil_impressao where modo_venda_id = 0)
  end
